...
/** Represents a note in frequency calculation.
  ...
  * @param harmonics
  *   an [[Iterable]] of frequencies that represents the harmonics in [20,
  *   20000]Hz.
  *
  * @author
  *   wildpants
...
case class Note(
  ...
  stdFreq: Double,
  fund: Double,
  harmonics: Iterable[Double]
):

  /** Get the prettified string representation for console output. (with ANSI
    * color)
    *
    * @example
  ...
  def displayFormat(): String =
    StringBuilder(endl)
      .append {
        "---- Harmonic series of '%s'".format(
          this.name.displayName.underlined.yellow.bold
        )
      }
      .append { endl }
      .append {
        "---- Standard A = [%s Hz]".format(this.stdFreq.toString.bold.yellow)
      }
      .append { endl * 2 }
      .append {
        this.harmonics
          .map(freq =>
            freq match
              case this.fund =>
                ">>   [%sHz]".format(f"$freq%-19.2f").underlined.bold.yellow
