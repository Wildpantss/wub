...
private[parser] object ProductParsingUtils:
  ...
  /** Parse the input arguments into 'T' instance.
    ...
    * @return
    *   an [[Either]], [[Left]] that contains error message when failed to
    *   parse, otherwise a [[Right]] contains the 'T' instace
    */
  inline def readProduct[T](args: Seq[String])(using
    m: Prd[T]
  ): Either[String, T] =
    ...
    checkArgLength(args.size, defaults, reads.size) match
      ...
      case None =>
        ...
        val readLefts =
          partitioned._1.asInstanceOf[List[(Left[String, _], String)]]
        val readRights =
          partitioned._2.asInstanceOf[List[(Right[String, _], String)]]
        readLefts match
          case head :: next =>
            Left(
              s"unable to parse argument '${head._2}', reason: ${head._1.swap.getOrElse("")}"
            )
          case Nil =>
            val processedVals =
              defaults.zipAll(readRights, None, (Right(""), "")).map {
                case None -> r => r._1.getOrElse("")
                case Some(d) -> r =>
                  r._1 match { case Right("") => d; case Right(v) => v }
              }
  ...
  /** Check the length of input arguments.
    ...
    * @return
    *   an [[Option]], contains a [[String]] as error message if counts of input
    *   arguments are invalid
    */
  private inline def checkArgLength(
    len: Int,
    defVal: Seq[Option[_]],
    arity: Int
  ): Option[String] =
    ...
    else if cntLb == cntUb then
      Some(s"$cntLb arguments required, actual received: $len")
    else Some(s"$cntLb to $cntUb arguments required, actual received: $len")

  /** A transformer for [[Seq]] to [[Product]], the API caller should never use
    * this directly.
    *
    * Since we're using deriving to get information from the [[Product]] you use
    * to configurate input arguments, there are a lot of details are implemented
    * by `inline` methods. the [[SeqProduct]] is required in the intermediate
    * processes, it is neccessary for it to be visible at the `inline` callsite.
    */
