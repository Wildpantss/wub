  ...
  val argInfoList: Expr[List[ArgInfo]] =
    caseSym.primaryConstructor.paramSymss match
      ...
      case head :: tails =>
        ...
        val descs = '{
          $names.map(n => $argDescs.find(_._1 == n).map(_._2).getOrElse(""))
        }
        '{
          ($names lazyZip $argDefaults lazyZip $descs).map(
            (name, default, desc) =>
              default match {
...
private[parser] inline def getDefaults[T]: List[Option[Any]] =
  ${ getDefaultsImpl[T] }

private[internal] def getDefaultsImpl[T: Type](using
  Quotes
): Expr[List[Option[Any]]] =
  import quotes.reflect.*
...
private[parser] inline def getCaseFieldNames[T]: List[String] =
  ${ getCaseFieldNamesImpl[T] }

private[internal] def getCaseFieldNamesImpl[T: Type](using
  Quotes
): Expr[List[String]] =
  import quotes.reflect.*
...
private[internal] def docStringToSingleLine(raw: Expr[String])(using
  Quotes
): Expr[String] =
  Expr {
    raw.valueOrAbort
...
private[internal] def splitSingleLine(singleLine: Expr[String])(using Quotes) =
  singleLine.valueOrAbort.split("@param").toList match
    ...
    case head :: tails =>
      Expr(head.strip) -> Expr.ofList(tails.map(tail =>
        handleParamString(Expr(tail))
      ))

private[internal] def handleParamString(p: Expr[String])(using
  Quotes
): Expr[(String, String)] =
