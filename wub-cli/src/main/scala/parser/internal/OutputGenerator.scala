...
private[parser] class OutputGenerator(
  ...
  def commandHelp(cmd: String): String =
    ...
    argDetails.foreach(a =>
      builder.append { s"%-${maxNameLen + 6}s %s".format(a._1, a._2) + endl }
    )
  ...
  def cmdNotFoundInfo(cmd: String): String = StringBuilder(endl)
    .append {
      (s"command '$cmd' not found! please choose one from follow:" <<< Red) + endl
    }
    .append {
      taskInfo.toList.map(_._1).map(x => s" - ${x <<< CMD_NAME_STYLE}").reduce(
        _ + endl + _
      )
    }
    .append { endl }
