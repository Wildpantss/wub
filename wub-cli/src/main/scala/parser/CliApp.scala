...
class CliApp private (
  ...
  private val tasks: Map[String, CliCmd] =
    setupHelpAndVersion(rawTasks, rawTaskInfo)._1
  private val taskInfo: Map[String, CmdInfo] =
    setupHelpAndVersion(rawTasks, rawTaskInfo)._2
  private val outputGen: OutputGenerator =
    OutputGenerator(appName, version, author, desc, taskInfo)
  ...
  private def processSuccess(cmd: String): Unit =
    () // no extra works when success
  ...
  private def setupHelpAndVersion(
    tasks: Map[String, CliCmd],
    taskInfo: Map[String, CmdInfo]
  ) =
    val (versionTask, versionInfo) =
      task[Version](_ => Right(printLine(outputGen.versionInfo)))
...
object CliApp:
  ...
  /** A type-aliase for function that represents the cli task.
    *
    * It takes a [[Seq]] of [[String]] as input from console, returns an
    * [[Either]]: [[Left]] with error message when failed, [[Right]] of [[Unit]]
    * when success.
    */
...
/** Shortcut for register a task.
  ...
  *   [[apply]]
  */
inline def task[T: Mirror.ProductOf](func: T => Either[String, Unit])
  : (CliCmd, CmdInfo) =
  (
    (args: Seq[String]) => readProduct[T](args).flatMap(x => func(x))
  ) -> inspectCmd[T]
