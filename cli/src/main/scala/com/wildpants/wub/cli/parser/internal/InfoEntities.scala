...
case class CmdInfo(name: String, desc: String, args: List[ArgInfo]):
  ...
  /** Get the usage information of this command.
    *
    * This methods returns an intermediate representation, so the call-site is able to control the
    * format more freely.
    *
  ...
  def usageColored: (String, String) =
    val argListColored = args.map(_.nameDispColored).reduceOption(_ + " " + _).getOrElse("")
...
enum ArgInfo(val name: String, val desc: String):
  ...
  /** Information of a required CLI-Argument.
    ...
    */
  case Req(override val name: String, override val desc: String) extends ArgInfo(name, desc)
  ...
  /** Information of a optional CLI-Argument.
    ...
    *   the description of argument
    */
  case Opt(override val name: String, override val desc: String) extends ArgInfo(name, desc)
